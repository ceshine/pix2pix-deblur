...
def get_grad_pen(Dis_net, real_data, fake_data, lmbda):
    epsilon = t.FloatTensor(real_data.size(1), real_data.size(
        2), real_data.size(3)).uniform_(0, 1)
    ...
    gradients = t.autograd.grad(outputs=Dis_net(interpolated_dataV).mean(0).view(
        1), inputs=interpolated_dataV, create_graph=True, retain_graph=True, only_inputs=True)[0]
...
def calc_gradient_penalty(netD, real_data, fake_data, _lambda=.1):
    alpha = torch.rand(real_data.size()[0], 1, 1, 1)
    alpha = alpha.expand_as(real_data).contiguous()
    alpha = alpha.cuda()
    ...
    gradients = grad(outputs=disc_interpolates, inputs=interpolates,
                     ...
                     # torch.FloatTensor(*disc_interpolates.size()).fill_(1),
                     create_graph=True, retain_graph=True, only_inputs=True)[0]

    gradient_penalty = ((gradients.norm(2, dim=1) - 1).pow(2)).mean() * _lambda
